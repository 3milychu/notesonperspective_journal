...
function loadReactions() {
	d3.json('data/vid3_new_moment_all2.json')
	.then(function(data){
		console.log(data)
		getReactions(data);
...
// draw signatures
function getSignature(data, role, offset=0){
	...
	if(data['rater1_reassigned']>=3){
		if(sentiment == "pos"){
			drawPath(point1, point2, "sig_pos", comment, offset);
		} else if( sentiment == "neg") {
			drawPath(point1, point2, "sig_neg", comment, offset);
		} else {
			drawPath(point1, point2, "sig_mixed", comment, offset)
		}
	} else {
		if(sentiment == "pos"){
	...
	if(data['rater2_reassigned']>=3){
		if(sentiment == "pos"){
			drawPath(point1, point3, "sig_pos", comment, offset);
		} else if(sentiment == "neg") {
			drawPath(point1, point3, "sig_neg", comment, offset);
		} else {
			drawPath(point1, point3, "sig_mixed", comment,offset)
		}
	} else {
		if(sentiment == "pos"){
